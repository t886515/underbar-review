(function() {
  ...
  _.last = function(array, n) {
    //n undefined - return last
    //n is 0 - return empty []
    //n is > length - entire array
    //n < length - from n to the end
    if (n === undefined) {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    //if theres accumulator start with that as the var otherwise use collection[0]
      // each every single item in collection,
        //pass accumulator and iterator
        //add
  ...
  _.every = function(collection, iterator) {
    ...
    // reduce (array, function(acc, item) {if (false) {}iterator(item) === true, [].push() return acc}, true)
    iterator? iterator : iterator = _.identity;
    var x = _.reduce(collection, function(acc, item) {
      if (!acc) {
        return false;
      }
      if (iterator(item)) {
  ...
  _.memoize = function(func) {
    ...
    return function() {
      var keys = JSON.stringify(arguments);
      if (!memoObj[keys]) {
        memoObj[keys] = func.apply(this, arguments);
        return memoObj[keys];
      } else {
        return memoObj[keys];
      }
